...
Generator.prototype.attach = function(options) {
  ...
  _.extend(router, {
     ...
     **/
    url: function(route, params, host) {
      params = params || {};

      var uri = url.parse('');
      debugger;
      var routeObject = this.routes[route];
      ...
      // allow for host to be past directly when windowless
      var _host = host || null;
      var _port = null;

      if (_host != null) {
        var host_split = _host.split(":");
      ...
      uri.hostname = routeObject.hostname || location.hostname || _host;
      uri.port = routeObject.hasOwnProperty('port') ? routeObject.port : (location.port || _port);
