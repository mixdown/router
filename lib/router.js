...
Router.prototype.dispatch = function(request, response) {
  ...
  if (controller.authenticate) {
    this._authenticate(httpContext, function(err) {

      if (!httpContext.user.is_logged_in && app.error) {
        app.error.notAuthenticated(new Error('Please log in.'), httpContext);
        return;

      } else if (!httpContext.user.is_logged_in) {
        httpContext.response.statusCode = 401;
        httpContext.response.end('Not Authenticated');
      } else {
        execute();
      }
