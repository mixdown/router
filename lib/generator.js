...
var simplifyRoutes = function(routes) {
  var clean = _.cloneDeep(routes);

  _.each(clean, function(v) {
...
Generator.prototype.attach = function (options) {
  ...
  **/
  ...
  var router = this.router = {
    ...
    **/
    url: function(route, params) {
      ...
      var singlepage = routeObject.singlepage ? true: false;

      if (!routeObject) {
        throw new Error('Route not found: ' + route);
      }
      
      // if these are set on the route and "singlepage" is not true, then attach them.  This will allow injection of FQ urls.
  ...
  , lightRoutes: simplifyRoutes(options.routes)
