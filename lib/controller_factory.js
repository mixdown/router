...
var ControllerFactory = function(options) {
  ...
  if (options.routes) {
    ...
    this.paths = _.map(options.paths, function(p) {
      ...
      if (p.add_namespace && typeof(p.url_prefix) === 'undefined') {
...
ControllerFactory.prototype.init = function(done) {
  ...
  if (this.paths) {
    ...
    this._manifest = this.routes;

    // load the controllers from the config.
    this.controllers = {};
    _.each(this._manifest, function(route_config, route_name) {
...
ControllerFactory.prototype.manifest = function() {
...
ControllerFactory.prototype._crawl = function(file_path, callback) {
  ...
  if (typeof(file_path.authorize) === 'string') {
    file_path.authorize = [file_path.authorize];
  }
  ...
  fs.exists(controllersPath, function(exists) {
    ...
    if (!exists) {
      ...
      callback(new Error('Controllers path does not exist.  path: ' + controllersPath));
      return;
    }
        ...
        return function(cb) {
          ...
          fs.stat(full_path, function(err, stats) {
            ...
            cb(null, _.extend(stats, {
              namespace: file_path.add_namespace ? (file_path.namespace || path.basename(controllersPath)) : null,
              file: file,
              full_path: full_path,
      ...
      async.parallel(ops, function(err, results) {
        ...
        _.each(cdirs, function(dir) {
          ...
          if (rconfig) {
            ...
            if (typeof(route_config.authenticate) === 'undefined') {
              route_config.authenticate = file_path.authenticate;
            }

            if (typeof(route_config.authorize) === 'string') {
              route_config.authorize = [route_config.authorize];
            }

            if (typeof(route_config.authorize) === 'undefined') {
              route_config.authorize = file_path.authorize;

            } else if (_.isArray(route_config.authorize)) {
              route_config.authorize = route_config.authorize.concat(file_path.authorize);
            }
...
ControllerFactory.prototype.load_manifest = function(callback) {
  ...
  this.crawl(function(err) {
    ...
    _.each(self.controllers, function(c, name) {
      self._manifest[name] = c.manifest();
    });
